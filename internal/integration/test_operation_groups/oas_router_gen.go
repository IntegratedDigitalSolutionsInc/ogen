...
func (s *ImagesServer) cutPrefix(path string) (string, bool) {
	prefix := s.cfg.Prefix
	if prefix == "" {
		return path, true
	}
...
func (s *ImagesServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	elem := r.URL.Path
	elemIsEscaped := false
	...
	default:
		...
		case '/': // Prefix: "/images"
			...
			if len(elem) == 0 {
				...
				case "POST":
					s.handleUploadImageRequest([0]string{}, elemIsEscaped, w, r)
				default:
...
func (s *ImagesServer) FindRoute(method, path string) (Route, bool) {
	return s.FindPath(method, &url.URL{Path: path})
}

// FindPath finds Route for given method and URL.
func (s *ImagesServer) FindPath(method string, u *url.URL) (r Route, _ bool) {
	var (
		elem = u.Path
		args = r.args
	)
		...
		case '/': // Prefix: "/images"
			...
			if len(elem) == 0 {
				...
				case "GET":
					...
					r.summary = "List all images"
					r.operationID = "listImages"
					r.pathPattern = "/images"
					r.args = args
					r.count = 0
					return r, true
				case "POST":
					r.name = UploadImageOperation
					r.summary = "Upload a new image"
					r.operationID = "uploadImage"
					r.pathPattern = "/images"
					r.args = args
					r.count = 0
					return r, true
...
func (s *UsersServer) cutPrefix(path string) (string, bool) {
	prefix := s.cfg.Prefix
	if prefix == "" {
		return path, true
	}
...
//
// Note: this method does not unescape path or handle reserved characters in path properly. Use FindPath instead.
func (s *UsersServer) FindRoute(method, path string) (Route, bool) {
	return s.FindPath(method, &url.URL{Path: path})
}

// FindPath finds Route for given method and URL.
func (s *UsersServer) FindPath(method string, u *url.URL) (r Route, _ bool) {
	var (
		elem = u.Path
		args = r.args
	)
